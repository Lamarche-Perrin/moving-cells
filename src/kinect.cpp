...
void Kinect::init ()
{
  ...
  struct timeval kinectStartTimer;
  gettimeofday (&kinectStartTimer, NULL);

  stop = false;

  if (allowSensorDisplay) cv::namedWindow ("depth");
...
void Kinect::run ()
{
  while (! stop)
	...
	{
	  thresholdKinect = false;
	  thresholdKinectStep = thresholdSteps;
	  if (thresholdFrame != 0) { delete thresholdFrame; }
	  thresholdFrame = new cv::Mat(424, 512, CV_32FC1, float(0));
	}

      if (thresholdKinectStep > 0)
	{
	  std::cout << "THRESHOLDING (" << thresholdKinectStep << ")" << std::endl;
	  
	  for (unsigned int x = 0; x < depth->width; x++)
	    for (unsigned int y = 0; y < depth->height; y++)
	      {
		float d1 = thresholdFrame->at<float>(cv::Point(x,y));
		float d2 = 0;
		if (depthFrame->at<float>(cv::Point(x,y)) > 0) d2 = depthFrame->at<float>(cv::Point(x,y)) - thresholdAdd;
		if (d1 == 0 || (d2 != 0 && d1 > d2)) thresholdFrame->at<float>(cv::Point(x,y)) = d2;
	      }
	  
	  thresholdKinectStep--;

	  if (thresholdKinectStep == 0 && thresholdBlurSize > 0)
	    {
	      cv::GaussianBlur (*thresholdFrame, *thresholdFrame, cv::Size (thresholdBlurSize, thresholdBlurSize), thresholdBlurVar);
	    }
      ...
      // cv::imshow ("depth", *thresholdFrame / 4500.);
      // cv::waitKey (0);
      ...
      if (thresholdFrame != 0)
	{
	  ...
	  for (unsigned int i = 0; i < depth->width*depth->height; i++)
	    if ((tPixel[i] != 0 && dPixel[i] > tPixel[i]) || (distanceMax != 0 && dPixel[i] > distanceMax) || (distanceMin != 0 && dPixel[i] < distanceMin)) { dPixel[i] = 0; }

	  cv::Mat *newDepthFrame = new cv::Mat (depth->height, depth->width, CV_32FC1, double(0));
	  depthFrame->copyTo (*newDepthFrame);
	  float *newDPixel = newDepthFrame->ptr<float>(0);
	  
	  for (unsigned int x = 0; x < depth->width; x++)
	    for (unsigned int y = 0; y < depth->height; y++)
	      {
		int i = x+y*depth->width;
		if ((x == 0 || x == depth->width-1 || y == 0 || y == depth->height-1) ||
		    (newDPixel[i] > 0 &&
		     (dPixel[(x-1)+(y)*depth->width] == 0 ||
		      dPixel[(x+1)+(y)*depth->width] == 0 ||
		      dPixel[(x)+(y-1)*depth->width] == 0 ||
		      dPixel[(x)+(y+1)*depth->width] == 0)
		     )
		    )
		  { newDPixel[i] = 0; }
	  ...
	  dPixel = depthFrame->ptr<float>(0);
	}

      libfreenect2::Frame *undepth = NULL;
      //cv::Mat *undepthFrame = NULL;
      if (realPositioning) {
	undepth = new libfreenect2::Frame (depthWidth, depthHeight, 4);
	registration->undistortDepth (depth, undepth);
	//undepthFrame = new cv::Mat (depth->height, depth->width, CV_32FC1, undepth->data);
      }

      // COMPUTE POINTS AND MEASURES
      if (allowPointExtraction)
	{
	  newPointList = new PointList();
	  extractPoints (dPixel, undepth);
      ...
      if (allowMeasures) { computeMeasures (dPixel, undepth); }

      // DETECT OBJECTS
      if (allowObjectDetection)
	{
      ...
      if (key > 0)
	{
      ...
      delete depthFrame;
      if (realPositioning) { delete undepth; }
    }
}
...
{
  // if (allowMeasures) displayMeasures (depthFrame);
  if (allowObjectDetection) displayObjects (depthFrame);
...
void Kinect::extractPoints (float *dPixel, libfreenect2::Frame *undepth)
{
  for (int y = 0; y < depthHeight; y++)
      ...
      for (int x = 0; x < depthWidth; x++)
	{
	  if (dPixel[i] > 0)
		...
		newPointList->push_back (Point (rx, ry, rz));
	    }
	  i++;
	}
    }
...
void Kinect::computeMeasures (float *dPixel, libfreenect2::Frame *undepth)
{
  measureNb = 0;
...
void Kinect::displayMeasures (cv::Mat *depthFrame)
{
  if (measureNb > 0)
    {
      printf("%f %f\n", meanX, meanY);
      //depthFrame->at<float>(cv::Point(meanX,meanY)) = 4500;

      // for (int y = -20; y <= 20; y++) depthFrame->at<float>(cv::Point(meanX,y)) = 4500;
      // for (int x = -20; x <= 20; x++) depthFrame->at<float>(cv::Point(x,meanY)) = 4500;
      
      // int zMoyy = (meanZ * depthHeight) / 400; if (zMoyy >= depthHeight) { zMoyy--; }
      // for (int x = 0; x <= 20; x++) depthFrame->at<float>(cv::Point(x,zMoyy)) = 4500;
    }
}


void Kinect::displayObjects (cv::Mat *depthFrame)
{
  if (realPositioning) {
