...
// 	float dx, dy, dz;
// 	int rX, rY;
// 	Particle () : x (0), y (0), z (0), dx (0), dy (0), dz (0), rX (0), rY (0), rZ (0) {}
// };
...
// PROGRAM PARAMETERS
	static const int maxParticleNumber   = 1000000;
...
// PHYSICS VARIABLES
	...
	float rPixelSize;
	float doubleCloudWidth;
	float doubleCloudHeight;
	float doubleCloudDepth;

// PARTICLE VARIABLES
	std::vector<cv::Point3f> particlePosition;
	std::vector<cv::Point3f> particleSpeed;
	std::vector<cv::Point2f> particlePixel;
...
// THREAD VARIABLES
	...
	static void *updateParticles (void *args);
	void updateParticles (int id);

	static void *applyParticles (void *args);
	void applyParticles (int id);
