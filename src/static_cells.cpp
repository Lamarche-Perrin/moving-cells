...
void Cloud::computeParticles ()
{
	...
	for (int i = 0; i < threadNumber; i++)
	{
		ArgStruct args (this, i);
		int rc = pthread_create (&threads[i], NULL, &Cloud::clearPixels, (void *) &args);
	...
	{
		ArgStruct args (this, i);
		int rc = pthread_create (&threads[i], NULL, &Cloud::updateAndMoveParticles, (void *) &args);
		if (rc) { std::cout << "Error:unable to create thread," << rc << std::endl; exit(-1); }
	}
	...
	{
		ArgStruct args (this, i);
		int rc = pthread_create (&threads[i], NULL, &Cloud::applyPixels, (void *) &args);
		if (rc) { std::cout << "Error:unable to create thread," << rc << std::endl; exit(-1); }
	}
	...
	while (SDL_PollEvent (&event))
	{
		...
		case SDL_MOUSEBUTTONDOWN :
			...
			case SDL_BUTTON_LEFT :
				if (bodyWeight != 0) { events.interrupt (new InstantaneousVariation (this, BODY_WEIGHT, 0)); }
...
void Cloud::updateAndMoveParticles (int id)
{
	for (int i = firstParticle[id]; i < lastParticle[id]; i++) {
		...
		// Compute motion
		float distanceX = particle->x - bodyX;
		float distanceY = particle->y - bodyY;
	
		float factor = pow (pow (distanceX, 2) + pow (distanceY, 2), rGravitationFactor);
		if (factor == 0) return;

		float angle = atan2 (distanceY, distanceX);
		float ddx = - bodyWeight * cos (angle + rGravitationAngle) / factor - rParticleDamping * particle->dx;
		float ddy = - bodyWeight * sin (angle + rGravitationAngle) / factor - rParticleDamping * particle->dy;
